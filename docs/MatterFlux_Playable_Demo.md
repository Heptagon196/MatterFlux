...
- 一块始终存在、带碰撞的土壤底层；
- 以出生点附近 512×384 个 8 cm 地形像素作为启动缓存；这不是地图边界。缓存外任意
  世界格仍按“地图 seed + 全局格坐标”采样多频 Perlin 噪声并量化到 8 cm 高度台阶，
  因而地形会随玩家移动确定性地继续延伸；
...
服务器只复制 `MapSeed`，不会复制几十万份地形顶点。服务器和客户端都调用同一个
`BuildLevelLayout`；区块中的每个地形格都只依赖 seed 和全局格坐标，因此生成顺序、
玩家进入方向和本地缓存命中与否都不会改变结果。出生区域使用缓存值，缓存外使用同一
`FMath::PerlinNoise2D` 公式即时求值，两者在边界上连续。森林布局仍使用
`FRandomStream` 保证出生区域装饰一致。
地形不再为每个像素建立一个 Cube/HISM instance；每个 64×64 块把顶面和裸露侧面
合并到三个 `UProceduralMeshComponent` material section。相邻同高度、同材质的顶面
会用贪心矩形精确合并，形状不变但显著减少渲染和 Chaos 碰撞三角形；同一网格仍做精确静态
...
无限地形不会一次驻留在内存中。材质模拟使用 64×64 格块，在 9 块硬预算内以 round-robin
覆盖所有权威玩家的焦点及邻环；单人时等价于焦点附近 3×3 块；
静态地形使用 64×64 像素块，为每位玩家保留玩家中心半径 4 和镜头远端半径 4 的两套
9×9 窗口；镜头窗口向固定斜俯视方向 `+X/-Y` 偏移两块。单人两套窗口合并后是
113 块，同时提供可见网格和复杂静态碰撞。其外还有半径 5 的预取环，完整并集为
161 块；默认 LRU 热缓存为 192 块。
服务器
合并所有已控制 Pawn 的窗口，客户端只跟随本地玩家，避免第二位玩家离开首位玩家后
失去地形、碰撞或可破坏装饰。离开窗口的块会立即关闭
渲染和碰撞，但仍保留在有界 LRU 热缓存中，
回头时无需重新生成网格和烘焙碰撞；超过上限的最旧非激活块才会释放。
新预取块使用异步 Chaos cooking，并按每帧最多 2 块、8 ms 预算渐进生成；跨越区块边界
只更新目标集合，不会在一帧同步创建一整排地形。远距离传送仍应配合加载界面。
...
当前出生区域森林的显示代理块数低于默认 128 块热缓存上限，因此代理三角剖分在加载界面内
预热；移动跨块只切换完成网格的可见性。更大的未来地图超过该上限后自动保留按需构建，
不会为了消除尖峰而无界预建整张世界。

目前“无限”首先覆盖可行走地形高度场。出生区域外的花草树木、河流初始填充和可破坏
Source 仍等待迁移到独立的 seed→chunk 内容规则；所以远行不会遇到地形尽头，但离开
出生生态区后装饰密度会下降。动态修改必须继续由服务器复制，不能像未修改地形那样只靠
seed 重算。
...
自动化用固定 seed 创建 512×384 出生缓存并启用无限地形域，生成一个被
PlayerController 控制的真实角色，
...
2026-08-17 扩大到 113 块可见窗口、161 块预取窗口后，本机最终门禁结果如下。若仍在
边界帧同步生成整排区块，同一测试会测到约 439～524 ms；启用贪心顶面和分帧预取后：

| 速度 | 平均帧 | P95 | 区块边界 Tick 最大 |
|---|---:|---:|---:|
| 500 cm/s | 1.93 ms | 9.41 ms | 5.71 ms |
| 1200 cm/s | 2.41 ms | 9.02 ms | 1.30 ms |
| 2500 cm/s | 4.69 ms | 11.97 ms | 8.08 ms |

三档都跨越 9 次边界并通过 60 FPS 门禁；这些数据来自 NullRHI commandlet，主要反映
游戏线程生成、流式和碰撞管理成本，不代表实际 GPU 帧率。
